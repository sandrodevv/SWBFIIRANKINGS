...
from apps.rankings.services.duelist_discord import (
    send_eu_duelist_weekly_winners_notification,
)


class Command(BaseCommand):
    help = (
        "Send the previous-week top EU duelist winners Discord webhook "
    ...
    def handle(self, *args, **options):
        result = send_eu_duelist_weekly_winners_notification(force=options["force"])
        styles = {
        ...
        self.stdout.write(
            style(f"EU duelist Discord notification result: {result}")
        )
